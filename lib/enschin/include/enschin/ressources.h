...
public:
    ...
    void free() const;
    Model& getModel(const std::string& key) const{ return const_cast<Model &>(models.at(key)); }
    SpriteSheet& getSpriteSheet(const std::string& key) const { return const_cast<SpriteSheet &>(spriteSheets.at(key)); }
    Color& getColor(const std::string& key) const { return const_cast<Color &>(colors.at(key)); }
    Sprite& getSprite(const std::string& key) const { return const_cast<Sprite &>(sprites.at(key)); }
